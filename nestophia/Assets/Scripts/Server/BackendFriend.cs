...
public class BackendFriend
{
    ...
    public void SendFriendRequest(string nickname)
    {
        ...
        {
            Debug.LogError("유저 이름 검색 도중 에러 발생 : " + inDateBro);
            return;
        }
        ...
        {
            Debug.LogError($"{inDate} 친구 요청 도중 에러 발생 : " + friendBro);
            return;
        }
    ...
    {
        // Step 3. 친구 요청 불러오기 및 수락하기 (불러오기 부분)
    ...
    {
        // Step 3. 친구 요청 불러오기 및 수락하기 (수락하기 부분)
        ...
        if (bro.IsSuccess() == false)
        {
            Debug.LogError("친구 수락 중 에러 발생 : " + bro);
            return;
        }
